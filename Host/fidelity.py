...
class FidelityAutomation:
    ...
    def __init__(self, headless: bool = True, debug: bool = False, title: str = None, source_account: str = None,
        ...
        self.page.set_default_navigation_timeout(360000)  # 360 seconds for navigation
    ...
    def get_total_networth_and_daychange(self):
        """
        Scrapes the total net worth and day change from the Balances tab.
        Returns (total_networth: float, day_change: float)
        """
        try:
            # Go to balances tab
            self.page.goto("https://digital.fidelity.com/ftgw/digital/portfolio/balances", timeout=60000)
            self.wait_for_loading_sign(timeout=120000)
            self.page.wait_for_timeout(20000)  # Let page layout finish

            # Find Total Net Worth
            networth_elem = self.page.query_selector('div.total-balance__value')
            if not networth_elem:
                raise Exception("Could not find total net worth element")
            total_text = networth_elem.inner_text().strip().replace("$", "").replace(",", "")
            # Only try to parse if it's numeric
            try:
                total = float(total_text)
            except Exception:
                print("Total Net Worth element contained non-numeric text:", total_text)
                total = None

            # Day Change: Find by class, but skip non-numeric or header values
            day_change = None
            elems = self.page.query_selector_all('div.day-change')
            for elem in elems:
                val = elem.inner_text().strip().replace("$", "").replace(",", "")
                # Ignore if it's just a label/header
                try:
                    if val and all(c in "0123456789.-()" for c in val.replace("(", "").replace(")", "")):
                        # Handle parens for negatives (e.g., (123.45))
                        if val.startswith("(") and val.endswith(")"):
                            change = -float(val[1:-1])
                        else:
                            change = float(val)
                        day_change = change
                        break
                except Exception as exc:
                    print("Day change parse error:", exc)
                    continue

            if day_change is None:
                print("Could not find valid day change value, defaulting to 0.0")
                day_change = 0.0

            return total, day_change
        except Exception as e:
            print("Error in get_total_networth_and_daychange:", e)
            return None, 0.0
    ...
    def set_account_dict(self, account_num: str, balance: float = None, withdrawal_balance: float = None,
                         nickname: str = None, stocks: list = None, overwrite: bool = False):
        """
    ...
    def login(self, username: str, password: str, totp_secret: str = None, save_device: bool = True) -> bool:       
        ...
        try:
            ...
            print("CDebug Past loading signs. Attempting to get to summary page")
            ...
            if "login" in self.page.url:
                print("CDebug Found 2FA page")
                self.wait_for_loading_sign()
                widget = self.page.locator("#dom-widget div").first
                print("CDebug widget found but not visible")
                widget.wait_for(timeout=50000, state='visible')
                print("CDebug widget visible")
                ...
                if (totp_secret is not None and
                    ...
                    print("CDebug got to summary page!!!")
                ...
                if self.page.get_by_role("link", name="Try another way").is_visible(timeout=0):
                    print("CDebug trying 2FA via text")
                    if save_device:
                        self.page.locator("label").filter(has_text="Don't ask me again on this").check(timeout=0)
                        if (not self.page.locator("label").filter(has_text="Don't ask me again on this").is_checked(timeout=0)):
                            raise Exception("Cannot check 'Don't ask me again on this device' box")
    ...
    def login_2FA(self, code: str, save_device: bool = True):
        ...
        try:
            ...
            print("CDebug filled in 2FA code")
    ...
    def transaction(self, stock: str, quantity: float, action: str, account: str, dry: bool = True,
        ...
        try:
            ...
            try:
                ...
                try:
                    error_message = (
                        self.page.get_by_label("Error").locator("div").filter(has_text="critical").nth(2).text_content(
                            timeout=2000))
                ...
                if error_message == "":
                    try:
                        error_message = self.page.wait_for_selector('.pvd-inline-alert__content font[color="red"]',
                                                                    timeout=2000).text_content()
                        self.page.get_by_role("button", name="Close dialog").click()
                        error_box_closed = True
                    except Exception:
                        pass
    ...
    def open_account(self, type: typing.Optional[Literal["roth", "brokerage"]]) -> bool:
        ...
        try:
            if type == "roth":
                ...
                self.page.goto(
                    url="https://digital.fidelity.com/ftgw/digital/aox/RothIRAccountOpening/PersonalInformation")
                self.wait_for_loading_sign()
            ...
            if type == "brokerage":
                ...
                self.page.goto(
                    url="https://digital.fidelity.com/ftgw/digital/aox/BrokerageAccountOpening/JointSelectionPage")
                self.wait_for_loading_sign()
    ...
    def wait_for_loading_sign(self, timeout: int = 30000):
        ...
        signs = [self.page.locator("div:nth-child(2) > .loading-spinner-mask-after").first,
                 ...
                 self.page.locator(
                     ".pvd3-spinner-root > .pvd-spinner__spinner > .pvd-spinner__visual > div > .pvd-spinner__mask-inner").first,
                 ]
        for sign in signs:
    ...
    def nickname_account(self, account_number: str, nickname: str):
        ...
        try:
            ...
            if self.page.get_by_test_id("ap143528-account-customize-open-button").get_by_label(
                    "Customize Accounts").is_visible():
                # New view detected
                new_view = True
            ...
            # Enter the new name
            if new_view:
                self.page.get_by_test_id("ap143528-account-customize-account-input").get_by_role("textbox").fill(
                    nickname)
            else:
